...
5. **共通設定を一元化**
   `assets/config.js` でフロントエンド全体の設定をまとめ、既存ページとの重複定義を解消しました。

「何が入っているのか」をまず把握したいときは、上の5点だけ覚えておけば大丈夫です。
...
- **assets/config.js**: GoogleシートやAIエンドポイント、公式サイトURLなどの設定を一元管理。`window.CONFIG` と `window.FAQ_CONFIG` の両方に同じ情報を提供します。
- **contact.html**: FAQで解決できない場合の問い合わせフォーム。Apps Script エンドポイントにPOSTして担当へ通知します。
...
- `assets/config.js` を更新すると、FAQページ/検索ページ/AI検索ページ/問い合わせフォームすべてに同じ設定が行き渡ります。
  - `SHEET_ID` / `SHEET_NAME`: FAQデータを読み込むGoogleスプレッドシートの指定。
